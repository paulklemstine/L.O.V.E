...
2.  **`broadcast_capabilities` Method:** A new public method was added to compile and broadcast the local instance's capabilities (version, creator status, hardware info) to all connected peers. This is triggered by the `map_network` command in `jules.py`.
