...
async def static_analysis_node(state: DeepAgentState) -> Dict[str, Any]:
    ...
    # Handle WorkingMemory as either Pydantic model or dict
    raw_wm = state.get("working_memory", {})
    if isinstance(raw_wm, WorkingMemory):
        working_memory = raw_wm.active_variables
    elif hasattr(raw_wm, 'model_dump'):
        working_memory = raw_wm.model_dump().get('active_variables', {})
    elif isinstance(raw_wm, dict):
        working_memory = raw_wm
    else:
        working_memory = {}
    ...
    # Prepare updated working memory as dict for return
    if feedback_lines:
        ...
        return {
            "messages": [],  # No new message needed if passing silently
            "working_memory": WorkingMemory(active_variables=new_active_vars)
        }
