...
while true; do
    ...
    if [ -f ".ralph/STOP" ]; then
        echo "🛑 Stop signal detected. Pausing Ralph Loop..." | tee -a $LOG_FILE
        break
    fi
    ...
    # The runner exits after completing one task
    # We use tee to show output in real-time while logging it
    python3 task_runner.py 2>&1 | tee -a $LOG_FILE
    EXIT_CODE=${PIPESTATUS[0]} # Get exit code of the python process, not tee
